...
void Image::Merge(Image* background, const int x, const int y) {
    ...
    //  skidoo    howdy.  Can possibly nix the following memset line
 ...
 * background, the background must contain the image.
 */
//   #d efine IMG_POS_RGB(p, x) (3 * p + x)
//    ^---v
//     skidoo todo followup to cull this unused fn
/*
...
// Fill the image with the given color and adjust its dimensions to passed values.
...
Image::createPixmap(Display* dpy, int scr, Window win) {
    ...
    case TrueColor: {
        ...
        break;
    default: {
            logStream << "Login.app: could not load/parse imagefile" << endl;
            return(tmp);
        }
    }
...
int
Image::readJpeg(const char *filename, int *width, int *height, unsigned char **rgb) {
    int ret = 0;
    ...
    if (infile == NULL) {
        logStream << "SLiM: Cannot fopen jpeg file: " << filename << endl;
        return ret;
    }
    ...
    {
        logStream << "SLiM: Unreasonable dimension found in jpeg file: "
                  << filename << endl;
        goto close_file;
    }
    ...
    *height = cinfo.output_height;
    ...
    if (rgb[0] == NULL) {
        logStream << "SLiM: Can't allocate memory for JPEG file."
                  << endl;
        goto close_file;
    }
               ...
               unsigned char **rgb, unsigned char **alpha)
{
    ...
    FILE *infile = fopen(filename, "rb");
    if (infile == NULL) {
        logStream << "SLiM: Cannot fopen PNG file: " << filename << endl;
        return ret;
    }
...
*/
    ...
    if(w >= MAX_DIMENSION || h >= MAX_DIMENSION) {
        logStream << "SLiM: Unreasonably large dimension found in PNG file: "
                  << filename << endl;
        goto png_destroy;
    }
    ...
    *height = (int) h;
        ...
        || color_type == PNG_COLOR_TYPE_GRAY_ALPHA)
    {
        ...
        if (alpha[0] == NULL) {
            logStream << "SLiM: Can't allocate memory for alpha channel in PNG file."
                    << endl;
            goto png_destroy;
        }
    }
    ...
    if (row_pointers == NULL) {
        logStream << "SLiM: Can't allocate memory for PNG file." << endl;
        goto png_destroy;
    }
    ...
    for (i = 0; i < *height; i++) {
        ...
        if (row_pointers == NULL) {
            logStream << "SLiM: Can't allocate memory for PNG file."
                      << endl;
            goto rows_free;
        }
    }
